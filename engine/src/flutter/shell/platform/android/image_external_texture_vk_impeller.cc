...
void ImageExternalTextureVKImpeller::ProcessFrame(PaintContext& context,
  ...
  if (direct.buffer != nullptr) {
    ...
    IngestHardwareBuffer(direct.buffer, release_ack_fd);
    return;
  }
  ...
  IngestHardwareBuffer(AHardwareBufferFor(hardware_buffer), /*release_ack_fd=*/-1);
}

void ImageExternalTextureVKImpeller::IngestHardwareBuffer(
    AHardwareBuffer* ahb,
    int release_ack_fd) {
  ...
  if (texture == nullptr) {
    auto texture_source = std::make_shared<impeller::AHBTextureSourceVK>(
        impeller_context_, ahb, hb_desc.value());
